// Shared daily signal-alert digest logic (Resend + Supabase alert_subscriptions).
...
/** Run digest and return a plain result object (not API Gateway shape). */
async function runAlertDigest() {
  ...
  try {
    ...
    throw new Error('Supabase: ' + (err.message || err));
  }
  ...
  if (subErr) {
    throw new Error('alert_subscriptions: ' + subErr.message);
  }

  if (!subs || !subs.length) {
    return { sent: 0, note: 'No subscribers in alert_subscriptions yet.' };
  }
  ...
  const feeds = await loadFeeds(site);
  ...
  for (const sub of subs.slice(0, MAX_EMAILS_PER_RUN)) {
    ...
    try {
      const result = await sendResendEmail({ apiKey, from, to: email, subject, html });
      if (result.ok) sent += 1;
