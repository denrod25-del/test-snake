...
- DeedScout is the main brand (target domain `deedscout.app`, Netlify at `https://deedscout.netlify.app/`). Root `index.html` is the marketing homepage using shared `assets/deedscout.css` + `deedscout.js`; it only forwards to `tax-deeds.html` when the URL has `?query` or `#hash` (Supabase auth / Stripe callbacks). Phase 1+2 marketing pages: `pricing.html`, `about.html`, `investor-pitch.html`, `contractor-intelligence.html`, `tax-deed-investors.html`, `contact.html`, `subdivisions-plats.html`. Core SaaS app is `tax-deeds.html` (Supabase auth + Stripe Pro at $29/mo via `netlify/functions/`; schema in `supabase/schema.sql`; Supabase Auth Site URL and redirect URLs must use the Netlify site host; parallel Vite app in `app/`). Legal pages use DeedScout branding; contact via Netlify form on `contact.html`.
...
- `scripts/build_county_pages.py` generates SEO county pages under `counties/*.html` (~71 pages) plus `data/counties/stats.json`; the homepage loads stats.json for live county badges (next sale date + surplus portal).
